...
| ⚠ A picture stacked behind the page is one we bury ourselves | `z-index: -1` on an in-flow image wrapper is an everyday idiom — it is how a card puts its cover under the layer that has to stay clickable — and it works only because everything above it is transparent. Painting order is what makes that fragile: a negative-z child is drawn at step 2 of its nearest ancestor **stacking context**, while the grounds of ordinary block descendants are drawn at step 3, above it. That context is `<html>` whenever nothing between is positioned with a z-index of its own, so every box we paint between the picture and the root buries it — `<body>` alone is enough, which means even `bg ground` on its own does it. The bookshop's grid lost every cover this way and kept the text around them, so unpainting the click target above only got half of it back. The repair goes on the **parent**, and the choice of element is the whole argument: raising the picture's own z-index would fix this and break the mirror case, a full-bleed backdrop at `position: absolute; inset: 0; z-index: -1` lifted over the text it belongs behind. A stacking context around the card traps the picture there instead — above that card's ground, still below its siblings, out of reach of every ancestor. `isolation: isolate` rather than `position: relative; z-index: 0` because it makes the context without becoming a containing block for anything positioned inside, and it leaves `position: fixed` alone, which only transform, filter and will-change disturb. The cost is that a positioned descendant can no longer escape a box holding a picture directly — a dropdown hanging out of such a card could fall behind the next one, which is cosmetic against a cover that is simply gone | `tools/make-default-styles.py` |
| ⚠ A strip of chrome is not a surface | A carousel's nav is one `<div>` stretched over the entire viewport of the carousel at `position: absolute; inset: 0; z-index: 9; pointer-events: none`, holding the prev and next buttons and nothing else — chrome **for** the picture underneath, click-through precisely so the picture stays reachable. Painted at (1,0,0) it is an opaque sheet across the photo, and one such element blanked five pictures on a shop's product page: the main photo, the thumbnail row and two recommendation carousels. `pointer-events: none` also hides it from `elementsFromPoint`, so a hit test at the centre of the photo reports the `<img>` on top with nothing above it — only a paint diff finds it. Neither handle `ui: overlays` had could reach it: not `:empty` (it holds the two buttons), and named nothing a list could match (`carousel-navs carousel-def car-load-hide abs`). CSS cannot select on `pointer-events`, the same wall the padding row and the triangle hit, so the structure carries it — holds a control, holds **nothing but** controls, holds no picture, and lies beside one, the same sibling test the whole-card click target uses. `:has(> control)` is not redundant beside `:not(:has(> :not(control)))`: the second is vacuously true of an element with no element children, so every `:empty` layer would match without it. And "children" means **element** children — `:has()` cannot see a text node, so a row of prose with a button in it is matched too, which costs it only a ground its ancestors already supply. `LINK_BUTTONS` is deliberately out: widening to links took a page-grounded `bottombar` and a pagination row, and a box holding only links is a menu | `tools/make-default-styles.py` |
| ⚠ A hover fill is a painter too | The whole-card click target held at rest and went the moment the mouse crossed it: a daily's opener photo, with one `<a>` laid across it at `position: absolute; width: 100%; height: 100%; z-index: 2` (its own background a transparent 1×1 GIF, holding nothing but a gallery badge), turned into an opaque cyan sheet on hover — cyan on a photo reads as white. `ui: links` fills a hovered link at (1,2,1), and the card-link repair in `ui: overlays` sat at a single guard, (1,0,3): enough for the bg blankets, and nothing else had been thought about. The general statement is that **a hover fill paints exactly as `bg all` does, and nothing `ui: overlays` leaves transparent may take it**. Two edits follow. The descendant hover form `a:hover *` had carried a mechanical doubled guard at (2,2,1) since the first version — above the named-overlay rule at (2,1,0), so a hovered Material UI row filled its own ripple span and the label went behind a cyan sheet, and above every other repair in that style; it only ever needed to beat the text blankets at (1,0,1) and its own rest form by order, so it is one guard now, (1,2,1). And the card link goes to a doubled guard, (2,2,4), which makes explicit the two things the old weight was losing to on purpose: `:not(:empty)`, since an empty link is a wordmark and keeps `ui: image-ground`'s grey at (1,1,1), and `:not(LINK_BUTTONS)`, since a link naming itself a button keeps its `ui: controls` pill at (2,1,1) and its yellow hover at (2,2,1). The ink has to come back with the ground: the fill also turns the text black, which on a now-transparent title link beside a picture is black on black, so a hovered twin restores cyan — hover only, so `:visited` keeps magenta at rest, and the link alone, since `a:hover *` filling the badge inside is the hover cue. The fixture tests hover by rewriting `:hover` to a class of the same weight, (0,1,0), so the cascade is the real one | `tools/make-default-styles.py` |
